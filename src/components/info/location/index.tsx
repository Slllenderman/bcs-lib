...
export const ConferenceLocation = (props: iprops.ConferenceLoactionProps) => {
    ...
    return (
        ...
        <ThemeProvider theme={theme}>
            <Container variant="accent" sx={{width:`${1200 * (props.width ? props.width : 1)}px`}}>
                <Box display="flex" flexDirection="row" alignItems="center" width={1}>
                    <Box display="flex" flexDirection="column" marginLeft={`${8 * (props.width ? props.width : 1)}%`} width="50%">
                        <Typography variant="subtitle1" noWrap={true} marginBottom={`${10 * (props.height ? props.height : 1)}%`} fontK={props.font_size}>
                            Место проведения
                        </Typography>
                        <Typography variant="h2" marginBottom={`${10 * (props.height ? props.height : 1)}%`} fontK={props.font_size}>
                            { props.address }
                        </Typography>
                        <Box width="80%">
                            <Button variant="outlined" href={props.ya_link}>
                                Открыть схему проезда
                            </Button>
                        </Box>
                    </Box>
                    <Box width={`${50 * (props.height ? props.height : 1)}%`} height="100%" marginLeft="auto">
                        <svg viewBox="0 0 592 424" width="100%" height="100%" fill="none">
